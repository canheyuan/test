...
var getAllFriend = function (app,cbOK, cbErr) {
  var options = {
    'From_Account': app.data.fromUser.id,
    'TimeStamp': 0,
    'StartIndex': 0,
    'GetCount': 50,
...
//发送消息(文本或者表情)
function onSendMsg(that,app,msgtosend,sucFn,errFn) {
  var loginInfo = app.data.fromUser;
  var selToID = app.data.toUser.id;
  if (!selToID) {
    that.setData({ textMessage:'' }); //清空文本框
    return;
  }
  //获取消息内容
...
function sendMsg(msg, sucFn ,errFn) {
  webim.sendMsg(msg, function (resp) {
    ...
    // showMsg(msg);
    // }
    //webim.Tool.setCookie("tmpmsg_" + app.data.toUserId, '', 0);
    console.debug("发消息成功");
    sucFn && sucFn(resp)
  }, function (err) {
...
//获取历史消息
function getMsgFn(callback) {
  var app = getApp();
  var toUserId = app.data.toUser.id
  ...
  webim.getC2CHistoryMsgs(
    ...
    function (resp) {
      ...
      console.log('最后获取到的消息列表：', msgArr);
      //消息已读上报，并将当前会话的消息设置成自动已读
      webim.setAutoRead(selSess, true, true);
      if (callback) { callback(textMsg); };
    },
    function (res) {
...
function convertMsg(app,msg) {
  ...
  var loginInfo = app.data.fromUser;//自己的资料
  var friendInfo = app.data.toUser;//对方的资料，这里要特别注意一下，消息里面是不会返回双方的头像和昵称的，只能通过指定的方法得到。
  elems = msg.getElems();
